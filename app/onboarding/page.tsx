...
export default function OnboardingPage() {
        ...
        case 1:
          return (
            <div className="space-y-2">
              ...
              <p className="text-white text-sm">Business: {onboardingData.business}</p>
              <p className="text-white text-sm">Country: {onboardingData.country}</p>
            </div>
          );
        case 2:
          return (
            <div className="space-y-2">
              <p className="text-blue-400 text-sm">✓ Completed</p>
              <p className="text-white text-sm">Ad Account ID: {onboardingData.ad_account_ID}</p>
              <p className="text-white text-sm">Organization Email: {onboardingData.organization_email}</p>
            </div>
          );
        default:
          return null;
    ...
    return (
      <div className="min-h-screen">
        <div className="mb-8">
          ...
          <p className="text-gray-400">You&apos;re all set up with Meta Ads Automation</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-gray-800/60 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50">
            <h3 className="text-lg font-semibold text-white mb-4">Ad Account Connected</h3>
            {getStepContent(2)}
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen">
      {/* Onboarding Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-white mb-2">Welcome to Meta Ads Automation</h1>
        <p className="text-gray-400">Let&apos;s get you set up with your first automated campaign</p>
      </div>

      {/* Onboarding Steps */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {/* Step 1: Business Profile */}
        <div className={`bg-gray-800/60 backdrop-blur-sm rounded-xl p-6 border ${
          getStepStatus(1) === 'completed' ? 'border-blue-500/50' : 'border-gray-700/50'
        }`}>
          <div className="flex items-center space-x-3 mb-4">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
              getStepStatus(1) === 'completed' ? 'bg-blue-600' : 'bg-blue-600/60'
            }`}>
          ...
          {getStepContent(1) || (
            <>
              <p className="text-gray-300 mb-4">Set up your business information and preferences</p>
              <button 
        ...
        <div className={`bg-gray-800/60 backdrop-blur-sm rounded-xl p-6 border ${
          getStepStatus(2) === 'completed' ? 'border-blue-500/50' : 'border-gray-700/50'
        }`}>
          <div className="flex items-center space-x-3 mb-4">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
              getStepStatus(2) === 'completed' ? 'bg-blue-600' : 'bg-blue-600/60'
            }`}>
              <span className="text-white font-bold">2</span>
            </div>
            <h3 className="text-lg font-semibold text-white">Connect Ad Account</h3>
          </div>
          
          {getStepContent(2) || (
            <>
              <p className="text-gray-300 mb-4">Connect your Facebook/Meta Ads account</p>
              <button 
        ...
        <div className={`bg-gray-800/60 backdrop-blur-sm rounded-xl p-6 border ${
          getStepStatus(3) === 'completed' ? 'border-blue-500/50' : 'border-gray-700/50'
        }`}>
          <div className="flex items-center space-x-3 mb-4">
            <div className={`w-8 h-8 rounded-full flex items-center justify-center ${
              getStepStatus(3) === 'completed' ? 'bg-blue-600' : 'bg-blue-600/60'
            }`}>
          ...
          {getStepContent(3) || (
            <>
              <p className="text-gray-300 mb-4">Review all provided information before proceeding</p>
              <button 
      ...
      <div className="mt-12">
        <div className="flex items-center justify-between mb-4">
          ...
          <span className="text-gray-400">{completedSteps.length}/3 Complete</span>
        </div>
